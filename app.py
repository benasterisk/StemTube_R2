...
# ------------------------------------------------------------------
# Jam Session state (in-memory, ephemeral)
# ------------------------------------------------------------------
active_jam_sessions = {}  # {code: {host_sid, host_user_id, host_name, extraction_id, extraction_data, participants, state, rtts, created_at}}
...
def handle_disconnect():
    ...
    for code, jam in list(active_jam_sessions.items()):
        if jam['host_sid'] == request.sid:
            ...
            socketio.emit('jam_host_status', {'status': 'disconnected', 'timeout': 10}, room=f'jam_{code}')

            # Schedule cleanup after 30s server-side grace period
            def _check_host_timeout(session_code):
                socketio.sleep(30)
                session = active_jam_sessions.get(session_code)
                if session and session.get('host_disconnected_at'):
                    # Host did not reconnect within grace period
                    socketio.emit('jam_ended', {'reason': 'Host disconnected'}, room=f'jam_{session_code}')
                    del active_jam_sessions[session_code]
                    logger.info(f"[Jam] Session {session_code} ended (host timeout after 30s)")

            socketio.start_background_task(_check_host_timeout, code)
            break
...
def handle_jam_end(data):
    ...
    if not jam or jam['host_sid'] != request.sid:
        return
    socketio.emit('jam_ended', {'reason': 'Host ended session'}, room=f'jam_{code}')
    del active_jam_sessions[code]
    logger.info(f"[Jam] Session {code} ended by host (DB code preserved)")

@socketio.on('jam_delete_code')
def handle_jam_delete_code(data=None):
    ...
    if code:
        ...
        if code in active_jam_sessions:
            socketio.emit('jam_ended', {'reason': 'Host deleted session code'}, room=f'jam_{code}')
            del active_jam_sessions[code]
...
def handle_jam_track_load(data):
    ...
    socketio.emit('jam_track_loaded', {
        'extraction_id': data.get('extraction_id'),
        'extraction_data': data.get('extraction_data')
    }, room=f'jam_{code}', include_self=False)

@socketio.on('jam_playback')
def handle_jam_playback(data):
    ...
    socketio.emit('jam_playback', data, room=f'jam_{code}', include_self=False)

@socketio.on('jam_tempo')
def handle_jam_tempo(data):
    ...
    socketio.emit('jam_tempo', data, room=f'jam_{code}', include_self=False)

@socketio.on('jam_pitch')
def handle_jam_pitch(data):
    ...
    socketio.emit('jam_pitch', data, room=f'jam_{code}', include_self=False)

@socketio.on('jam_sync')
def handle_jam_sync(data):
    ...
    data['server_timestamp'] = time.time() * 1000
    socketio.emit('jam_sync', data, room=f'jam_{code}', include_self=False)

@socketio.on('jam_pong')
...
@app.route('/api/extractions/<extraction_id>/beats/regenerate', methods=['POST'])
@api_login_required
...
# ------------------------------------------------------------------
# Jam Session HTTP routes
# ------------------------------------------------------------------
@app.route('/api/jam/my-session')
@login_required
def jam_my_session():
...
@app.route('/api/jam/stems/<code>/<stem_name>', methods=['GET', 'HEAD'])
...
@app.route('/api/jam/extraction/<code>')
