...
from core.logging_config import log_request, get_logger, _ctx_logger
...
def setup_request_logging(app):
    ...
    def log_request_start():
        ...
        if not request.path.startswith('/static/'):
            ...
            ctx = _ctx_logger(logger, request_id=g.request_id, user_id=user_id, ip_address=g.client_ip)
            ctx.debug(f"Request started: {request.method} {request.path}")
    ...
    def log_request_end(response):
        ...
        if hasattr(g, 'request_start_time') and not request.path.startswith('/static/'):
            ...
            if is_error or is_slow:
                ...
                ctx = _ctx_logger(logger, request_id=g.request_id, user_id=user_id, duration=duration_ms)
                if is_slow:
                    ctx.warning(f"Slow request: {request.method} {request.path} took {duration_ms}ms")
                if is_error:
                    ctx.warning(f"Request error: {request.method} {request.path} -> {response.status_code}")
    ...
    def log_404_error(error):
        ...
        ctx = _ctx_logger(logger, user_id=user_id, ip_address=getattr(g, 'client_ip', None))
        ctx.warning(f"404 Not Found: {request.method} {request.path}")
    ...
    def log_500_error(error):
        ...
        ctx = _ctx_logger(logger, user_id=user_id, ip_address=getattr(g, 'client_ip', None))
        ctx.error(f"500 Internal Server Error: {request.method} {request.path}", exc_info=True)
...
def require_request_logging(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            ctx = _ctx_logger(logger, request_id=g.request_id, user_id=user_id)
            ctx.error(f"Exception in {f.__name__}: {str(e)}", exc_info=True)
            raise
...
def log_api_call(endpoint_name, user_id=None, **kwargs):
    ...
    ctx = _ctx_logger(logger, user_id=user_id, **kwargs)
    ctx.info(f"API call: {endpoint_name}")


def log_user_session_event(event_type, user_id, details=None):
    """Log user session events like login, logout, etc."""
    ctx = _ctx_logger(logger, user_id=user_id)
